...
#
#  CS_CHROME_NAME     Name of the installed sandbox (installation specific)
#  CS_CHROME_TOP      Path up to and including the sandbox's primary git project
#         (eg. this file is CS_CHROME_TOP/sandbox/defaults.sh)
#  CS_CHROME_SANDBOX  Path to the root directory of the sandbox tree
...
sandutil_load_options $CS_CHROME_SANDBOX || { echo "failed to load options" >&2 && return 1; }
...
# export CS_CHROME_YARN=true
# export CS_CHROME_YARN_VER=latest
# export PATH=$CS_CHROME_SANDBOX/yarn/bin:$PATH
...
export CS_CHROME_NODE_VER=12.14.1
export PATH=$CS_CHROME_SANDBOX/node/bin:$CS_CHROME_TOP/node_modules/.bin:$PATH


# Add $MY_SANDBOX/bin to the search path
export PATH=$CS_CHROME_TOP/bin:$PATH

# if you want to short circuit the sandbox hooks (see hooks/git_hooks.sh) either uncomment
# this in defaults.sh or add 'CS_CHROME_DISABLE_GIT_HOOKS=1' to CS_CHROME_SANDBOX/sb.options
# export CS_CHROME_DISABLE_GIT_HOOKS=1

# Standard variables for all sandboxes - USE THESE TO INTEGRATE WITH OPS!!!
export CS_CHROME_LOGS=$CS_CHROME_SANDBOX/log    # Log directory
export CS_CHROME_TMP=$CS_CHROME_SANDBOX/tmp     # temp directory
export CS_CHROME_CONFS=$CS_CHROME_SANDBOX/conf  # config files directory
export CS_CHROME_DATA=$CS_CHROME_SANDBOX/data   # data directory
export CS_CHROME_PIDS=$CS_CHROME_SANDBOX/pid    # pid files directory
...
[ -z "$CS_CHROME_ASSET_ENV" ] && export CS_CHROME_ASSET_ENV=local

# The sandbox run-time environment;  eg. 'local', 'qa', 'prod', 'loadtest1', ...
# https://github.com/TeamCodeStream/dev_tools/blob/master/README/README.deployments.md)
[ -z "$CS_CHROME_ENV" ] && export CS_CHROME_ENV=local
